...
func (c *Config) load() (diff bool, err error) {
	c.Once.Do(func() {
		flag.StringVar(&c.config, "c", "./zpclient.ini", "set configuration `file`")
		flag.Parse()
	})
	...
	// zpp_version
	iv, _ := strconv.ParseInt(globalConfig["zpp_version"], 10, 64)
	nIPPVersion := int(iv)
	...
	// local_proxy_port
	c.LocalProxyPort = globalConfig["local_proxy_port"]
